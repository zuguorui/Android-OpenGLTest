...
bool PicRender::init(int width, int height, PicTexture *texture) {
    backingLeft = 0;
    backingTop = 0;
    backingWidth = width;
...
void PicRender::resetRenderSize(int left, int top, int width, int height) {
    backingLeft = left;
    backingTop = top;
    backingWidth = width;
    backingHeight = height;
...
GLuint PicRender::compileShader(GLenum type, const char *source) {
    GLint status;
